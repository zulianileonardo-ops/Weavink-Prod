        ...
        <div className={`fixed bottom-0 left-0 right-0 ${run && (isFirstStep || stepIndex === 9) ? 'z-[9998]' : 'z-[10000]'} md:hidden transition-transform duration-300 ${isMapOpen ? 'translate-y-full' : 'translate-y-0'}`}>
            <div className="flex justify-around items-center py-3 px-2 m-2 rounded-2xl bg-white border shadow-lg backdrop-blur-lg">
                ...
                <Link
                    ...
                    className={`flex items-center justify-center p-3 active:scale-90 active:opacity-40 hover:bg-black hover:bg-opacity-[0.075] rounded-xl transition-all ${activePage === 6 ? "opacity-100 bg-black bg-opacity-5" : "opacity-50 hover:opacity-70"} ${eventsHighlight}`}
                    aria-label={translations.events}
                >
                    <svg className="w-[22px] h-[22px]" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
