...
function loadVectors() {
  console.log(chalk.blue('\n📦 Loading vectors from pinecone_export.json...\n'));
  ...
  console.log(chalk.green(`✅ Loaded ${data.length} vectors across ${Object.keys(byNamespace).length} namespaces\n`));
...
class QdrantTestClient {
  ...
  async connect() {
    ...
    try {
      console.log(chalk.blue(`🔗 Connecting to Qdrant at ${CONFIG.QDRANT_URL}...`));
      ...
      console.log(chalk.green(`✅ Connected to Qdrant at ${this.url}\n`));
      return;
    } catch (err) {
      console.log(chalk.yellow(`⚠️  Primary URL failed: ${err.message}`));
    }

    // Try fallback URL
    try {
      console.log(chalk.blue(`🔗 Trying fallback URL: ${CONFIG.QDRANT_FALLBACK_URL}...`));
   ...
   * Note: Qdrant retrieve() requires integer IDs, but Weavink uses string IDs.
   * We use scroll with filter as a workaround to find vectors by their stored ID in payload.
   */
  async fetchVectors(ids, collection) {
    const start = Date.now();

    // Use scroll with ID filter since retrieve() only works with integer point IDs
    // Weavink stores the contact ID in the payload, so we filter by that
    ...
    // Extract vectors and calculate cosine similarity (mimics real use case)
    const vectors = (results.points || []).map(r => r.vector);
    if (vectors.length >= 2) {
      const similarity = cosineSimilarity(vectors[0], vectors[1]);
    }
  ...
  async bulkUpsert(vectors, collection) {
    ...
    // Use UUID for point ID since Qdrant prefers UUID or integer IDs
    const points = vectors.map((v, index) => ({
      id: crypto.randomUUID(),  // Generate UUID for Qdrant point ID
      vector: v.values,
      payload: { ...v.metadata, id: v.id },  // Store original ID in payload
    }));
  ...
  /**
   * Ensure collection exists for upsert test
   */
  async ensureCollection(collection) {
    try {
      ...
      console.log(chalk.gray(`  Created Qdrant collection: ${collection}`));
    }
  }
}
...
class BenchmarkRunner {
  ...
  async runTest(testName, testFn, warmup = true) {
    ...
    for (let i = 0; i < totalIterations; i++) {
      ...
      try {
        ...
        console.log(chalk.red(`  ${iterLabel}: Error - ${err.message}`));
        throw err;
      }
    }
    ...
    console.log(chalk.green(`\n  🏆 Winner: ${winner} (${speedup.toFixed(1)}% faster on avg)`));
  }
  ...
  async testSimilaritySearch() {
    ...
    if (namespaces.length === 0) {
      console.log(chalk.yellow('⚠️  No namespaces found, skipping test'));
      return;
    }
    ...
    if (namespaceVectors.length < 2) {
      console.log(chalk.yellow('⚠️  Not enough vectors in namespace, skipping test'));
      return;
    }
  ...
  async testFetchVectors() {
    ...
    if (namespaceVectors.length < 2) {
      console.log(chalk.yellow('⚠️  Not enough vectors, skipping test'));
      return;
    }
    ...
    console.log(chalk.gray(`  Vector IDs: ${ids.join(', ')}`));
  ...
  async testBulkUpsert() {
    ...
    await this.qdrant.ensureCollection(testCollection);
  ...
  async testFilteredSearch() {
    ...
    const sampleMetadata = namespaceVectors[0].metadata || {};
    console.log(chalk.gray(`  Sample metadata fields: ${Object.keys(sampleMetadata).join(', ') || 'none'}`));
    ...
    await this.runTest('Filtered Search (With Metadata Filter)', async (db, iter) => {
      if (db === 'pinecone') {
        // Pinecone filter - check if any field exists
        const filter = Object.keys(sampleMetadata).length > 0
          ? { [Object.keys(sampleMetadata)[0]]: { $exists: true } }
          : {};
        return await this.pinecone.filteredSearch(queryVector, testNamespace, filter, 10);
      } else {
        // Qdrant filter - use a simple must_not to include all (baseline)
        // or filter by a specific field if available
        const filter = Object.keys(sampleMetadata).length > 0
          ? { must: [{ key: Object.keys(sampleMetadata)[0], match: { any: [sampleMetadata[Object.keys(sampleMetadata)[0]]] } }] }
          : {};
  ...
  printComparisonTable() {
    ...
    if (qdrantWins > pineconeWins) {
      console.log(chalk.green(`✅ Qdrant wins ${qdrantWins} out of ${Object.keys(this.results).length} tests`));
      console.log(chalk.gray('   → Qdrant is generally faster for your use cases'));
      console.log(chalk.gray('   → Consider migrating to self-hosted Qdrant for better performance'));
    } else if (pineconeWins > qdrantWins) {
      console.log(chalk.cyan(`✅ Pinecone wins ${pineconeWins} out of ${Object.keys(this.results).length} tests`));
      console.log(chalk.gray('   → Pinecone is generally faster for your use cases'));
      console.log(chalk.gray('   → Consider staying with Pinecone for managed service benefits'));
    } else {
      console.log(chalk.yellow(`⚖️  Tie: ${qdrantWins} wins each`));
      console.log(chalk.gray('   → Performance is comparable between both databases'));
      console.log(chalk.gray('   → Decision should be based on cost, operations, and self-hosting preference'));
    }
    ...
    // Cost comparison
    console.log(chalk.blue('\n💰 Cost Considerations:\n'));
    console.log(chalk.gray('   Pinecone (Serverless):'));
    console.log(chalk.gray('     - $2/million queries (read units)'));
    console.log(chalk.gray('     - $0.33/GB/month storage'));
    console.log(chalk.gray('     - No server management'));
    console.log(chalk.gray(''));
    console.log(chalk.gray('   Qdrant (Self-hosted on Hetzner):'));
    console.log(chalk.gray('     - Already included in your VPS cost'));
    console.log(chalk.gray('     - ~1GB RAM per 1M vectors'));
    console.log(chalk.gray('     - You manage backups/updates'));
...
async function main() {
  ...
  console.log(chalk.bold.blue('║        Pinecone vs Qdrant Benchmark for Weavink              ║'));
  console.log(chalk.bold.blue('╚════════════════════════════════════════════════════════════════╝\n'));
  ...
  try {
    ...
    // Initialize clients
    console.log(chalk.blue('🔧 Initializing database clients...\n'));
    ...
    console.log(chalk.gray(`\n  Qdrant collections: ${collections.join(', ')}`));
