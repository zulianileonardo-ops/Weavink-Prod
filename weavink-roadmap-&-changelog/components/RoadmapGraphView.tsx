...
interface GraphNode {
  id: string;
  label: string;
  type: 'root' | 'category' | 'subcategory' | 'commit' | 'issue';
  x: number;
  y: number;
  color: string;
  data?: any;
...
export const RoadmapGraphView: React.FC<Props> = ({ tree }) => {
  ...
  const { nodes, links, width, height } = useMemo(() => {
    ...
    const linkList: { sourceId: string; targetId: string }[] = [];
    ...
    const processNode = (
      ...
      depth: number,
      data: any,
      childNodesData: Array<{ id: string; label: string; type: GraphNode['type']; data?: any; children?: any[] }>
    ): number => {
