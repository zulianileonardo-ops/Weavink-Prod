...
export class MatchingService {
  ...
  /**
   * Calculate semantic similarity score using Pinecone vectors
   *
   * @param {string} contactId1 - Contact 1 ID
   * @param {string} contactId2 - Contact 2 ID
   * @param {string} userId - User ID for index lookup
   * @returns {Promise<number>} Score 0-1
   */
  static async calculateSemanticScore(contactId1, contactId2, userId) {
    try {
      // Get Pinecone index for user
      const index = IndexManagementService.getIndex(userId);
      if (!index) {
        console.log('[MatchingService] No Pinecone index available, skipping semantic score');
        return 0;
      }

      // Fetch both vectors
      const [vector1Response, vector2Response] = await Promise.all([
        index.fetch([contactId1]),
        index.fetch([contactId2]),
      ]);

      const vector1 = vector1Response.records?.[contactId1]?.values;
      const vector2 = vector2Response.records?.[contactId2]?.values;
