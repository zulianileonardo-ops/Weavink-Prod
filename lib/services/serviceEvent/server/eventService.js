...
export class EventService {
  ...
  static async createEvent({ eventData, session }) {
    ...
    console.log('[EventService] Event created in Firestore:', eventId);
  ...
  static async getUserEvents({ session, options = {} }) {
    ...
    // Apply date filters
    if (startAfter) {
      query = query.where('startDate', '>=', new Date(startAfter).toISOString());
    }

    if (startBefore) {
      query = query.where('startDate', '<=', new Date(startBefore).toISOString());
    }
    ...
    console.log('[EventService] Found events:', events.length);
  ...
  static async updateEvent({ eventId, eventData, session }) {
    ...
    const validation = validateEventData(updateData);
    if (!validation.isValid) {
      throw new Error(`Invalid event data: ${validation.errors.join(', ')}`);
    }
