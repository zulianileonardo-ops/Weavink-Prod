...
export class EventService {
  ...
  static async getUserEvents({ session, options = {} }) {
    ...
    // Add user's participation for each event
    const eventsWithParticipation = await Promise.all(
      events.map(async (event) => {
        const myParticipation = await this.getUserParticipation({
          eventId: event.id,
          userId: session.userId
        });
        return { ...event, myParticipation };
      })
    );
    ...
    return {
      ...
      hasMore: eventsWithParticipation.length === limit,
    };
  }
   ...
   * @returns {Promise<Object|null>} Participation data or null
   */
  static async getUserParticipation({ eventId, userId }) {
