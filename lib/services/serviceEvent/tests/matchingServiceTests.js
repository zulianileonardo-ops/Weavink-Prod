...
export async function runMatchingServiceTests(testUserId = `test-msvc-${Date.now()}`) {
  ...
  try {
    ...
    const { eventId } = await createTestEvent(testUserId, { name: 'Match Creation Test Event' });
  ...
  try {
    ...
    const { eventId } = await createTestEvent(testUserId, { name: 'Find Matches Test Event' });
    createdEventIds.push(eventId);
  ...
  try {
    ...
    const { eventId } = await createTestEvent(testUserId, { name: 'Ghost Inclusion Test' });
    createdEventIds.push(eventId);
  ...
  try {
    ...
    const { eventId } = await createTestEvent(testUserId, { name: 'Private Exclusion Test' });
    createdEventIds.push(eventId);
  ...
  try {
    ...
    const { eventId } = await createTestEvent(testUserId, { name: 'Pending Query Test' });
    createdEventIds.push(eventId);
