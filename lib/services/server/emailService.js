...
export class EmailService {
     ...
     * @returns {Promise<{success: boolean, messageId: string}>}
     */
    static async sendContactDeletionNoticeEmail(recipientEmail, recipientName, deletedUserName, scheduledDate, requestId, locale = 'en') {
        try {
            const translations = loadTranslations(locale);
     ...
     * @returns {Promise<{success: boolean, messageId: string}>}
     */
    static async sendAccountDeletionCancelledEmail(recipientEmail, recipientName, requestId, locale = 'en') {
        try {
            const translations = loadTranslations(locale);
    ...
    /**
     * Send contact deletion cancellation notice email
     * @param {string} recipientEmail - Contact's email address
     * @param {string} recipientName - Contact's display name
     * @param {string} cancelledUserName - Name of person who cancelled deletion
     * @param {string} requestId - Deletion request ID
     * @param {string} locale - Contact's language preference
     * @returns {Promise<{success: boolean, messageId: string}>}
     */
    static async sendContactDeletionCancelledEmail(recipientEmail, recipientName, cancelledUserName, requestId, locale = 'en') {
        try {
            const translations = loadTranslations(locale);
            const t = translations.emails?.contact_deletion_cancelled || {};

            const subject = t.subject?.replace('{{userName}}', cancelledUserName) ||
                `Contact Deletion Cancelled - ${cancelledUserName}`;

            const htmlContent = this._generateContactDeletionCancelledTemplate(
                recipientName,
                cancelledUserName,
                requestId,
                locale,
            ...
            console.log(`✅ Contact deletion cancellation notice sent to: ${recipientEmail} (${locale})`);
            return {
                success: true,
                messageId: result.data?.messageId
            };
        } catch (error) {
            console.error('❌ Failed to send contact deletion cancellation notice:', error);
            throw new Error(`Failed to send contact deletion cancellation notice: ${error.message}`);
        }
    }
                  ...
                  <tr>
                    <td valign="top">
                      <table cellpadding="0" cellspacing="0" width="100%">
                        <tr>
                          <td>
                            <img alt="TapIt Logo" width="155" src="https://firebasestorage.googleapis.com/v0/b/tapit-dev-e0eed.firebasestorage.app/o/Images-Weavink%2Ffull-logo.png?alt=media&token=1ca917c6-cf13-43df-9efa-567b6e6b97b0" style="padding: 1rem">
                          </td>
                        </tr>
                        <tr>
...
<tbody style="margin-top: 15px;">
  <tr class="">
<td class="">
<img alt="TapIt Logo" class="" height="155" src="https://firebasestorage.googleapis.com/v0/b/tapit-dev-e0eed.firebasestorage.app/o/Images-Weavink%2Ffull-logo.png?alt=media&token=1ca917c6-cf13-43df-9efa-567b6e6b97b0">
</td>
</tr>
...
<tbody>
  <tr>
    <td>
      <img alt="TapIt Logo" height="80" src="https://firebasestorage.googleapis.com/v0/b/tapit-dev-e0eed.firebasestorage.app/o/Images-Weavink%2Ffull-logo.png?alt=media&token=1ca917c6-cf13-43df-9efa-567b6e6b97b0" style="margin-bottom: 20px;">
    </td>
  </tr>
        ...
        <tr>
          <td>
            ...
            ${buttonText ? `<div style="text-align: center;">
              <a class="button" href="${process.env.NEXT_PUBLIC_BASE_URL}/dashboard/account?tab=delete" target="_blank">
                ${buttonText}
              </a>
            </div>` : ''}
...
`;
     ...
     * @private
     */
    static _generateContactDeletionNoticeTemplate(recipientName, deletedUserName, scheduledDate, requestId, locale, translations) {
        const t = translations.emails?.contact_deletion_notice || {};
        const formattedDate = new Date(scheduledDate).toLocaleDateString(locale, {
            year: 'numeric',
            month: 'long',
...
`;
    ...
    static _generateAccountDeletionCompletedTemplate(recipientName, locale, translations) {
        ...
        const bestRegards = t.best_regards || 'Best regards,';
        const teamName = t.team_name || 'The Weavink Team';
      ...
      <tbody>
        <tr>
          <td>
            ...
            <div style="border-top: 1px solid #eee; padding-top: 30px; color: #666; font-size: 14px; margin-top: 30px;">
              <p>${bestRegards}<br><strong>${teamName}</strong></p>
              <p style="font-size: 12px; color: #999;">
...
`;
     ...
     * @private
     */
    static _generateAccountDeletionCancelledTemplate(recipientName, requestId, locale, translations) {
        const t = translations.emails?.account_deletion_cancelled || {};
        ...
        const welcomeBack = t.welcome_back || 'Welcome back!';
        const teamName = t.team_name || 'The Weavink Team';
        const requestIdLabel = t.request_id || 'Request ID:';
        const dpoLabel = t.dpo_label || 'Data Protection Officer:';
        ...
        <tr>
          <td>
            ...
            <div style="text-align: center;">
              <a class="button" href="${process.env.NEXT_PUBLIC_BASE_URL}/dashboard" target="_blank">
                ${buttonText}
              </a>
            </div>
            <div style="border-top: 1px solid #eee; padding-top: 30px; color: #666; font-size: 14px; margin-top: 30px;">
              <p>${welcomeBack}<br><strong>${teamName}</strong></p>
              <p style="font-size: 12px; color: #999;">
                ${requestIdLabel} ${requestId}<br>
                ${dpoLabel} dpo@weavink.io
              </p>
...
`;
    ...
    /**
     * Generates contact deletion cancellation email template
     * @private
     */
    static _generateContactDeletionCancelledTemplate(recipientName, cancelledUserName, requestId, locale, translations) {
        const t = translations.emails?.contact_deletion_cancelled || {};

        const greeting = t.greeting || 'Hi';
        const headline = t.headline || 'Contact Account Deletion Cancelled';
        const intro = (t.intro || `{{userName}}'s account deletion request has been cancelled.`)
            .replace('{{userName}}', cancelledUserName);
        const accountActive = t.account_active ||
            'Their Weavink account remains active and their contact information will not be anonymized.';
        const noActionNeeded = t.no_action_needed ||
            'No action is required from you. This is for your information only.';
        const teamName = t.team_name || 'The Weavink Team';
        const bestRegards = t.best_regards || 'Best regards,';
        const requestIdLabel = t.request_id || 'Request ID:';
        const dpoLabel = t.dpo_label || 'Data Protection Officer:';
