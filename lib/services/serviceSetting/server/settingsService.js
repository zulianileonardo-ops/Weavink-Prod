...
export class SettingsService {
    ...
    static async _validateLocationFeaturesTier(userId, locationFeatures) {
        try {
            ...
            const userData = userDoc.data();
            const subscriptionLevel = (userData.subscriptionLevel || 'base').toUpperCase();

            // Get allowed features for this tier
            const allowedFeatures = LOCATION_FEATURES_BY_TIER[subscriptionLevel];

            if (!allowedFeatures) {
                console.warn(`Unknown subscription level: ${subscriptionLevel}, defaulting to BASE`);
                const baseFeatures = LOCATION_FEATURES_BY_TIER['BASE'];
                allowedFeatures.geocoding = baseFeatures.geocoding;
                allowedFeatures.autoVenueEnrichment = baseFeatures.autoVenueEnrichment;
            }
