...
export class GitHubService {
  ...
  static async getCommitHistoryFromGitHub(options = {}) {
    const requestId = Math.random().toString(36).substring(7);
    console.log(`📊 [GitHubService:${requestId}] Fetching commit history from GitHub API`, options);
    ...
    // Check cache first
    const cacheKey = `commits_${owner}_${repo}_${options.limit}_${options.since}`;
    if (!options.forceRefresh) {
      const cached = cacheManager.get(cacheKey);
      if (cached) {
        console.log(`✅ [GitHubService:${requestId}] Returning cached commits (${cached.length})`);
        return cached;
      }
    }
    ...
    try {
      const octokit = this.getOctokit();
      const maxCommits = Math.min(options.limit || 500, ROADMAP_LIMITS.MAX_COMMITS);
      ...
      while (allCommits.length < maxCommits) {
        const commits = await this.fetchWithRetry(async () => {
          console.log(`🔍 [GitHubService:${requestId}] Fetching page ${page} from GitHub API...`);
        ...
        // Stop if we've reached the limit or got fewer than requested
        if (commits.length < perPage || allCommits.length >= maxCommits) {
          break;
        }
      }

      // Parse commits using shared utility
      ...
      console.log(`✅ [GitHubService:${requestId}] Fetched ${parsedCommits.length} commits`);
      ...
      // Cache results
      cacheManager.set(cacheKey, filtered, ROADMAP_LIMITS.GITHUB_CACHE_TTL);

      return filtered;

    } catch (error) {
      console.error(`💥 [GitHubService:${requestId}] Error fetching commits:`, error.message);
