...
import { ContactAnonymizationService } from './contactAnonymizationService.js';
import { VectorStorageService } from '../../serviceContact/server/vectorStorageService.js';
...
export async function executeAccountDeletion(userId, requestId) {
  try {
    ...
    try {
      ...
      console.log(`✅ Account deletion completed email sent to: ${userEmail} (${userLanguage})`);
    } catch (emailError) {
    ...
    try {
      // Find all users who had this person as a contact
...
async function removeContactFromUser(ownerUserId, deletedUserId, deletedEmail, deletedName) {
  try {
    ...
    if (!contactsDoc.exists) {
      console.log(`[ANONYMIZATION] No contacts document for user: ${ownerUserId}`);
      return;
    }
    ...
    if (contactIndex === -1) {
      console.log(`[ANONYMIZATION] No contact found for deleted user ${deletedUserId} in user ${ownerUserId}'s list`);
      return;
    }

    const contactToAnonymize = contacts[contactIndex];

    console.log(`[ANONYMIZATION] Anonymizing contact for deleted user ${deletedUserId} in user ${ownerUserId}'s contact list`);

    // === NEW: Comprehensive anonymization using ContactAnonymizationService ===
    ...
    // Update Firestore with anonymized contact
    await db.collection('Contacts').doc(ownerUserId).update({
      contacts,
      updatedAt: FieldValue.serverTimestamp(),
    });

    console.log(`[ANONYMIZATION] ✅ Contact anonymized in Firestore for user: ${ownerUserId}`);

    // === NEW: Anonymize Pinecone vector metadata (instead of deleting) ===
    try {
      await VectorStorageService.anonymizeVectorMetadata(
        ownerUserId,
        contactToAnonymize.id,
        anonymizedContact
      );
      console.log(`[ANONYMIZATION] ✅ Vector metadata anonymized for user: ${ownerUserId}`);
    } catch (vectorError) {
      console.error(`[ANONYMIZATION] ⚠️ Vector anonymization failed (non-critical):`, vectorError);
      // Don't throw - vector anonymization failure shouldn't block account deletion
    }

  } catch (error) {
    console.error(`[ANONYMIZATION] ❌ Error anonymizing contact for user ${ownerUserId}:`, error);
    // Don't throw - contact anonymization failure shouldn't block account deletion
  }
}
...
/**
 * Update notifications to reflect deletion completion
 * @param {string} deletedUserId - User ID that was deleted
 * @param {string} deletedEmail - Email of deleted user
 * @param {Array<string>} userIds - Array of user IDs whose notifications need updating
 * @param {string} deletedUserName - Name of deleted user
 * @returns {Promise<void>}
 */
    ...
    const updatePromises = userIds.map(async (userId) => {
      try {
        ...
        if (!querySnapshot.empty) {
          ...
          console.log(`✅ Updated notification for user ${userId}`);
        } else {
          console.log(`⚠️  No pending deletion notification found for user ${userId}`);
        }
      } catch (error) {
        console.error(`Error updating notification for user ${userId}:`, error);
        // Non-blocking: continue even if individual update fails
      }
    });

    await Promise.allSettled(updatePromises);
    console.log(`[AccountDeletion] Completed updating notifications`);
  } catch (error) {
    console.error('Error updating notifications:', error);
    // Non-blocking: don't fail the entire deletion if notifications fail
  }
}
