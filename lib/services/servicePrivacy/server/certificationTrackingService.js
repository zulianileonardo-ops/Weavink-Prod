...
export async function calculateProgress(certificationId) {
  try {
    ...
    const inProgressItems = checklist.filter(item => item.status === 'in_progress').length;
    const notStartedItems = checklist.filter(item => item.status === 'not_started' || !item.status).length;
    ...
    // Update the certification with the calculated progress
    await db.collection('Certifications').doc(certificationId).update({
      progress,
    ...
    console.error('[Certification] Error calculating progress:', error);
...
export async function getCertificationStatistics() {
  try {
    ...
    const stats = {
      totalCertifications: snapshot.size,
      byStatus: {},
      byType: {},
      averageProgress: 0,
      upcomingRenewals: 0,
...
/**
 * Get certification by ID (alias for getCertificationStatus)
 * @param {string} certificationId - Certification ID
 * @returns {Promise<Object>} Certification details
 */
export const getCertificationById = getCertificationStatus;
