...
import { reportSecurityIncident } from '../server/incidentReportingService.js';
...
async function runBreachNotificationTests(results) {
  ...
  if (testIncidentId) {
    ...
    try {
      ...
      const subjectNotif = await notifyDataSubjects(testIncidentId, {
        affectedUserIds: [TEST_USER_ID],
        notificationMethod: 'email',
      });

      const subjectValid = subjectNotif.success &&
        subjectNotif.notified >= 0;
