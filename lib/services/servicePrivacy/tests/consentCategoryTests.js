...
export async function runConsentCategoryTests(testUserId = 'test-user-consent-categories-001') {
  ...
  try {
    ...
    const result = await ConsentService.batchGrantConsents(
      testUserId,
  ...
  try {
    ...
    const result = await ConsentService.getUserConsents(testUserId);
  ...
  try {
    ...
    for (const type of CONSENT_CATEGORIES.AI_FEATURES.types) {
      await ConsentService.withdrawConsent(testUserId, type, {
        ipAddress: '192.168.1.1',
        userAgent: 'Mozilla/5.0 Test Browser',
      });
    }
    ...
    const aiSearchStatus = await ConsentService.hasConsent(testUserId, CONSENT_TYPES.AI_SEMANTIC_SEARCH);
    const aiGroupingStatus = await ConsentService.hasConsent(testUserId, CONSENT_TYPES.AI_AUTO_GROUPING);
    const aiCardStatus = await ConsentService.hasConsent(testUserId, CONSENT_TYPES.AI_BUSINESS_CARD_ENHANCEMENT);
  ...
  try {
    ...
    const result = await ConsentService.getConsentHistory(testUserId, { limit: 100 });
  ...
  try {
    ...
    const exportData = await ConsentService.exportConsentData(testUserId);
