...
export async function runConsentTests(testUserId = 'test-user-consent-001') {
  ...
  try {
    ...
    const hasMarketing = await ConsentService.hasConsent(testUserId, CONSENT_TYPES.MARKETING_EMAILS);
    const hasAnalytics = await ConsentService.hasConsent(testUserId, CONSENT_TYPES.ANALYTICS_DETAILED);
  ...
  try {
    ...
    const result = await ConsentService.batchGrantConsents(testUserId, consentsToGrant, {
      ipAddress: '192.168.1.1',
      userAgent: 'Mozilla/5.0 Test Browser',
    });
  ...
  try {
    ...
    const result = await ConsentService.withdrawConsent(testUserId, CONSENT_TYPES.MARKETING_EMAILS, {
      ipAddress: '192.168.1.1',
      userAgent: 'Mozilla/5.0 Test Browser',
    });
  ...
  try {
    ...
    const result = await ConsentService.exportConsentData(testUserId);
