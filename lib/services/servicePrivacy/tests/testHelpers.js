...
export async function deleteTestUser(userId) {
  try {
    ...
    // ✅ PERFORMANCE FIX: Run all batch deletions in parallel instead of sequentially
    // This reduces cleanup time from ~10-20s to ~2-4s for 7 test users
    await Promise.all([
      // Delete consent logs
      (async () => {
        const consentLogs = await db.collection('ConsentLogs').where('userId', '==', userId).get();
        if (!consentLogs.empty) {
          const batch = db.batch();
          consentLogs.forEach((doc) => batch.delete(doc.ref));
          await batch.commit();
        }
      })(),
      ...
      (async () => {
        ...
        if (!privacyRequests.empty) {
          const batch = db.batch();
          privacyRequests.forEach((doc) => batch.delete(doc.ref));
          await batch.commit();
        }
      })(),

      // Delete Phase 3 audit logs
      (async () => {
        const auditLogs = await db.collection('AuditLogs').where('userId', '==', userId).get();
        if (!auditLogs.empty) {
          const batch = db.batch();
          auditLogs.forEach((doc) => batch.delete(doc.ref));
          await batch.commit();
        }
      })(),

      // Delete Phase 3 legal holds
      (async () => {
        const legalHolds = await db.collection('LegalHolds').where('userId', '==', userId).get();
        if (!legalHolds.empty) {
          const batch = db.batch();
          legalHolds.forEach((doc) => batch.delete(doc.ref));
          await batch.commit();
        }
      })(),
    ]);
