  ...
  static async submitExchangeContact(submissionData) {
    ...
    try {
      ...
      if (contact.location && LocationEnrichmentService.isEnrichmentEnabled(userData)) {
        try {
          // ✨ PHASE 5: Multi-step session detection (geocoding + venue + tagging)
          ...
          const runnableSteps = [canGeocode, canEnrichVenue, canTag].filter(Boolean).length;
          const isMultiStep = runnableSteps >= 2;

          // Generate session ID ONLY for multi-step operations (2+ steps)
          ...
          if (enrichedContact.metadata?.venue) {
            ...
            // Finalize enrichment session (only for multi-step operations)
      ...
      if (AutoTaggingService.isAutoTaggingEnabled(userData) &&
        ...
        try {
          ...
          if (taggedContact.tags?.length > 0) {
            ...
            // Finalize session if it was created and tagging was part of multi-step
            if (enrichmentSessionId) {
              await SessionTrackingService.finalizeSession({
                userId: targetUserId,
                sessionId: enrichmentSessionId
              });
              console.log('🏁 [Exchange] Multi-step session finalized:', enrichmentSessionId);
            }
          }
        } catch (tagError) {
