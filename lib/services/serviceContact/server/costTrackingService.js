...
export class CostTrackingService {
  ...
  static async getUserMonthlyUsage(userId, usageType = 'AIUsage') {
    ...
    try {
      ...
      const maxCost = limits.aiCostBudget || 0;  // Shared budget for both AI and API operations
  ...
  static async canAffordGeneric(userId, usageType, estimatedCost, requiresBillableRun = false) {
    ...
    try {
      ...
      console.log(`🔍 [CostTracking] [${operationId}] Checking cost budget:`, {
        usageType,
        currentCost: usage.usage.totalCost,
        maxCost: usage.limits.maxCost,
        estimatedCost,
        projectedCost: usage.usage.totalCost + estimatedCost,
        guardCondition: usage.limits.maxCost > 0,
        wouldExceed: (usage.usage.totalCost + estimatedCost) > usage.limits.maxCost
      });
      ...
      if (wouldExceedBudget) {
        console.log(`🚫 [CostTracking] [${operationId}] Cost budget exceeded!`, {
          currentCost: usage.usage.totalCost,
          maxCost: usage.limits.maxCost,
          estimatedCost,
          remainingBudget: usage.remainingBudget
      ...
      console.log(`✅ [CostTracking] [${operationId}] Cost budget check passed`);

      if (requiresBillableRun) {
        console.log(`🔍 [CostTracking] [${operationId}] Checking run limits:`, {
          usageType,
          currentRuns: usage.usage.totalRuns,
          maxRuns: usage.limits.maxRuns,
          nextRunCount: usage.usage.totalRuns + 1,
          guardCondition: usage.limits.maxRuns > 0,
          wouldExceed: (usage.usage.totalRuns + 1) > usage.limits.maxRuns
        });
        ...
        if (wouldExceedRuns) {
          console.log(`🚫 [CostTracking] [${operationId}] Run limit exceeded!`, {
            currentRuns: usage.usage.totalRuns,
            maxRuns: usage.limits.maxRuns,
            remainingRuns: usage.remainingRuns
          });
        ...
        console.log(`✅ [CostTracking] [${operationId}] Run limit check passed`);
      }
