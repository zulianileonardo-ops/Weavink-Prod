...
export class LocationEnrichmentService {
    ...
    try {
      ...
      if (canGeocode) {
        ...
        await CostTrackingService.recordUsage({
          ...
          stepLabel: 'Reverse Geocoding',
          budgetCheck: apiAffordabilityCheck,  // Top-level parameter for session tracking
          metadata: {
            latitude,
            longitude,
      ...
      if (canEnrichVenue) {
        venueResult = await this.getVenueData(latitude, longitude, userId, sessionId, apiAffordabilityCheck);
      } else {
   ...
   */
  static async getVenueData(latitude, longitude, userId, sessionId = null, budgetCheck = null) {
    ...
    try {
      ...
      if (cached && cached.venue) {
        ...
        await CostTrackingService.recordUsage({
          ...
          sessionId: sessionId,  // null for standalone, sessionId for multi-step
          stepLabel: 'Venue Search (Cache Hit)',
    ...
    const venue = await this.searchNearbyVenue(latitude, longitude, userId, sessionId, budgetCheck);
   ...
   */
  static async searchNearbyVenue(latitude, longitude, userId, sessionId = null, budgetCheck = null) {
    ...
    try {
      ...
      const result = await PlacesService.searchNearbyVenues(userId, {
        ...
        budgetCheck    // Pass affordability check result for cost tracking
      });
