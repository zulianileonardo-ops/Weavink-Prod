...
class Neo4jClient {
  ...
  async read(cypher, params = {}) {
    ...
    const sessionTime = performance.now() - opStart;
    try {
      ...
      console.log(`⏱️ [Neo4j READ] ${totalTime.toFixed(2)}ms total | query: ${queryTime.toFixed(2)}ms | session: ${sessionTime.toFixed(2)}ms | records: ${records.length} | cypher: ${cypher.substring(0, 50).replace(/\s+/g, ' ')}...`);
  ...
  async write(cypher, params = {}) {
    ...
    try {
      ...
      console.log(`⏱️ [Neo4j WRITE] ${totalTime.toFixed(2)}ms total | query: ${queryTime.toFixed(2)}ms | session: ${sessionTime.toFixed(2)}ms | nodes: +${summary.nodesCreated}/-${summary.nodesDeleted} | rels: +${summary.relationshipsCreated}/-${summary.relationshipsDeleted}`);
  ...
  /**
   * Run latency benchmark - useful for comparing AuraDB vs Self-Hosted
   * @param {number} iterations - Number of operations per benchmark (default: 10)
   */
  async benchmark(iterations = 10) {
    const uri = process.env.NEO4J_URI || 'unknown';
    const isCloud = uri.includes('neo4j.io') || uri.includes('aura');

    console.log(`\n${'═'.repeat(70)}`);
    console.log(`  🏎️  NEO4J LATENCY BENCHMARK`);
    console.log(`${'═'.repeat(70)}`);
    console.log(`  URI: ${uri}`);
    console.log(`  Type: ${isCloud ? '☁️ Neo4j AuraDB' : '🏠 Self-Hosted'}`);
    console.log(`  Iterations: ${iterations}`);
    console.log(`${'─'.repeat(70)}`);

    try {
      await this.connect();
      const pingTimes = [], readTimes = [], writeTimes = [], deleteTimes = [];

      // Warm-up
      await this.read('RETURN 1');

      // PING benchmark (server info)
      console.log('  Running PING benchmark...');
      ...
      // READ benchmark (simple count)
      console.log('  Running READ benchmark...');
      for (let i = 0; i < iterations; i++) {
        const start = performance.now();
        await this.read('MATCH (n:BenchmarkNode) RETURN count(n) as count');
        readTimes.push(performance.now() - start);
      }

      // WRITE benchmark (create node)
      console.log('  Running WRITE benchmark...');
      const testId = `benchmark_${Date.now()}`;
      for (let i = 0; i < iterations; i++) {
        const start = performance.now();
        await this.write(
          'CREATE (n:BenchmarkNode {id: $id, iteration: $i, timestamp: datetime()}) RETURN n',
          { id: testId, i }
        );
      ...
      for (let i = 0; i < iterations; i++) {
        const start = performance.now();
        await this.write(
          'MATCH (n:BenchmarkNode {id: $id}) WITH n LIMIT 1 DELETE n RETURN count(*) as deleted',
          { id: testId }
        );
      ...
      // Cleanup remaining
      await this.write('MATCH (n:BenchmarkNode {id: $id}) DELETE n', { id: testId });
      ...
      const readStats = calcStats(readTimes);
      const writeStats = calcStats(writeTimes);
      ...
      console.log(`${'─'.repeat(70)}`);
      console.log(`  PING (info)  │ ${pingStats.min.padStart(6)}ms │ ${pingStats.avg.padStart(6)}ms │ ${pingStats.p50.padStart(6)}ms │ ${pingStats.p99.padStart(6)}ms │ ${pingStats.max.padStart(6)}ms`);
      console.log(`  READ (count) │ ${readStats.min.padStart(6)}ms │ ${readStats.avg.padStart(6)}ms │ ${readStats.p50.padStart(6)}ms │ ${readStats.p99.padStart(6)}ms │ ${readStats.max.padStart(6)}ms`);
      console.log(`  WRITE (node) │ ${writeStats.min.padStart(6)}ms │ ${writeStats.avg.padStart(6)}ms │ ${writeStats.p50.padStart(6)}ms │ ${writeStats.p99.padStart(6)}ms │ ${writeStats.max.padStart(6)}ms`);
      console.log(`  DELETE       │ ${deleteStats.min.padStart(6)}ms │ ${deleteStats.avg.padStart(6)}ms │ ${deleteStats.p50.padStart(6)}ms │ ${deleteStats.p99.padStart(6)}ms │ ${deleteStats.max.padStart(6)}ms`);
      console.log(`${'═'.repeat(70)}\n`);
      ...
      console.error('❌ [Neo4j] Benchmark error:', error.message);
      return null;
    }
  }
}
