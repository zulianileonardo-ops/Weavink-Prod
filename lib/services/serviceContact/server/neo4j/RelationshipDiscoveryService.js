...
// Integrates Neo4j graph with Qdrant semantic similarity
...
import { GroupNamingService } from './GroupNamingService.js';
 ...
 * 2. Semantic Similarity - Qdrant vector similarity
...
class RelationshipDiscoveryService {
  ...
  static async discoverAllRelationships(userId, contacts, options = {}) {
    ...
    try {
      ...
      // Step 5: Create KNOWS relationships from company clusters (95-98%)
      console.log('🤝 [RelationshipDiscovery] Step 5: Creating KNOWS relationships...');
      onProgress?.(95, 'Creating KNOWS relationships...');
      const knowsResult = await this.discoverKnowsRelationships(userId, contacts);
      results.knowsRelationships = knowsResult.relationshipsCreated;
      onProgress?.(98, `Created ${knowsResult.relationshipsCreated} KNOWS relationships`);
  ...
  /**
   * Discover semantic similarity using Qdrant vector search
  ...
  static async discoverSemanticSimilarity(userId, contacts, options = {}) {
    ...
    console.log(`   🔍 Querying Qdrant for semantic similarity (thresholds: high=${thresholds.high}, medium=${thresholds.medium}, low=${thresholds.low})`);
    ...
    try {
      // Get the Qdrant collection for this user
      ...
      for (const contact of contacts) {
        try {
          // Step 1: Fetch this contact's embedding vector from Qdrant
          const vectorResult = await qdrantClient.scroll(collectionName, {
            filter: {
          ...
          const contactVector = vectorResult.points?.[0]?.vector;
          ...
          const qdrantResults = await qdrantClient.search(collectionName, {
            ...
            limit: topK + 1,  // +1 to account for self-match
            with_payload: true,
            with_vector: false
          });

          // Step 3: Classify matches into confidence tiers
          for (const qdrantPoint of qdrantResults || []) {
            const matchContactId = qdrantPoint.payload?.originalId;

            // Skip self-match
            if (matchContactId === contact.id) continue;

            // Get confidence tier (returns null if below minimum threshold)
            const tier = getSemanticConfidenceTier(qdrantPoint.score);
            if (!tier) continue; // Below minimum threshold (< 0.20)

            // Avoid duplicate pairs (A->B and B->A)
            const pairKey = [contact.id, matchContactId].sort().join('|');
            ...
            const relationship = {
              ...
              targetId: matchContactId,
              targetName: qdrantPoint.payload?.name || contactNameMap.get(matchContactId) || 'Unknown',
              type: 'SIMILAR_TO',
              method: 'qdrant_embedding',
              score: qdrantPoint.score,
              confidence: tier,
              savedToNeo4j: false,
              llmAssessment: null
            };
  ...
  /**
   * Discover KNOWS relationships from company clusters
   * People working at the same company are assumed to know each other
   *
   * @param {string} userId - User ID
   * @param {Array} contacts - Contacts array (for context)
   */
  static async discoverKnowsRelationships(userId, contacts = []) {
    console.log(`   🤝 Creating KNOWS relationships from company clusters...`);

    let relationshipsCreated = 0;

    try {
      // Get company clusters - contacts working at the same company
      ...
      for (const cluster of companyClusters) {
        ...
        const memberCount = typeof cluster.memberCount === 'object' && 'low' in cluster.memberCount
          ? cluster.memberCount.low
          : Number(cluster.memberCount);

        // Only create KNOWS for companies with 2+ contacts
        if (memberCount >= 2) {
          const contactIds = cluster.members.map(m => m.id);
          const result = await neo4jClient.createCompanyKnowsRelationships(
            userId,
            contactIds,
            cluster.company
          );

          // Handle Neo4j integer format for created count
          const created = typeof result.created === 'object' && 'low' in result.created
            ? result.created.low
            : Number(result.created) || 0;

          relationshipsCreated += created;
          console.log(`      - ${cluster.company}: ${created} KNOWS edges`);
        }
      }

      console.log(`   ✅ KNOWS relationships complete: ${relationshipsCreated} created`);

      return {
        relationshipsCreated,
        companiesProcessed: companyClusters.length
      };

    } catch (error) {
      console.error(`   ❌ KNOWS relationship discovery failed:`, error.message);
      return {
        relationshipsCreated: 0,
        error: error.message
      };
   ...
   * Returns 4 types of suggestions: company, tag, semantic, knows
   *
   * @param {string} userId - User ID
   * @param {object} options - Options
   * @param {boolean} options.generateAINames - Whether to use AI naming (default: true)
   */
  static async getSuggestedGroups(userId, options = {}) {
    const suggestions = [];

    // 1. Company-based groups (from WORKS_AT relationships)
    const companyClusters = await neo4jClient.findCompanyClusters(userId);
    for (const cluster of companyClusters) {
      if (cluster.memberCount >= 2) {
        suggestions.push({
          type: 'company',
          name: `${cluster.company} Team`, // Will be replaced by AI name
          reason: `${cluster.memberCount} contacts work at ${cluster.company}`,
          members: cluster.members,
          confidence: 0.9,
          metadata: { company: cluster.company, domain: cluster.domain }
        });
      }
    }

    // 2. Tag-based groups (from HAS_TAG relationships)
    try {
      ...
      for (const cluster of tagClusters) {
        ...
        if (memberCount >= 2) {
          suggestions.push({
            type: 'tag',
            name: `${cluster.tag} Group`, // Will be replaced by AI name
            reason: `${memberCount} contacts tagged "${cluster.tag}"`,
            members: cluster.members,
            confidence: 0.8,
      ...
      console.error('❌ [getSuggestedGroups] Tag clusters failed:', error.message);
    }

    // 3. Similarity-based groups (from SIMILAR_TO relationships)
    ...
    for (const cluster of similarityClusters) {
      ...
      if (members.length >= 3) {
        suggestions.push({
          type: 'semantic',
          name: 'Similar Contacts', // Will be replaced by AI name
    ...
    // 4. Knows-based groups (from KNOWS relationships)
    try {
      const knowsClusters = await neo4jClient.findKnowsClusters(userId);
      const processedKnows = new Set();

      for (const cluster of knowsClusters) {
        if (processedKnows.has(cluster.contactId)) continue;
        ...
        if (members.length >= 3) {
          suggestions.push({
            type: 'knows',
            name: 'Connected Network', // Will be replaced by AI name
          ...
          members.forEach(m => processedKnows.add(m.id));
        }
      }
    } catch (error) {
      console.error('❌ [getSuggestedGroups] Knows clusters failed:', error.message);
    }

    // AI naming - generate creative names for all suggestions
    if (options.generateAINames !== false && suggestions.length > 0) {
      try {
        console.log(`📛 [getSuggestedGroups] Generating AI names for ${suggestions.length} suggestions...`);
        const namedSuggestions = await GroupNamingService.nameSuggestions(suggestions);
        return namedSuggestions;
      } catch (error) {
        console.error('❌ [getSuggestedGroups] AI naming failed, using fallback names:', error.message);
        // Return suggestions with fallback names on error
        return suggestions;
      }
    }
...
- Discovery Method: ${relationship.method === 'qdrant_embedding' ? 'Semantic similarity (AI-detected similar profiles)' : relationship.method === 'tags' ? `Shared tags: ${relationship.sharedTags?.join(', ') || 'N/A'}` : relationship.method}
