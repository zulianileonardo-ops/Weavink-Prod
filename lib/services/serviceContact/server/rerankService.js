  ...
  static async rerankContacts(query, contacts, options = {}) {
    ...
    try {
      ...
      if (rawCount > 0) {
        ...
        // === DIAGNOSTIC: Enhanced Score Distribution ===
        ...
        console.log(`🔍 [DIAGNOSTIC] [${rerankId}] Score Distribution Analysis:`, {
          total: sorted.length,
          buckets: scoreBuckets,
          percentiles: {
            p10: sorted[Math.floor(sorted.length * 0.1)]?.toFixed(4),
            p25: sorted[Math.floor(sorted.length * 0.25)]?.toFixed(4),
            p50: sorted[Math.floor(sorted.length * 0.5)]?.toFixed(4),
            p75: sorted[Math.floor(sorted.length * 0.75)]?.toFixed(4),
            p90: sorted[Math.floor(sorted.length * 0.9)]?.toFixed(4)
          },
          clustering: scoreBuckets.veryWeak > sorted.length * 0.8 ? 'CLUSTERED_LOW' : 'DISTRIBUTED'
        });
        ...
        // === DIAGNOSTIC: Top 5 YAML Documents ===
        console.log('');
        console.log(`🔍 [DIAGNOSTIC] [${rerankId}] Top 5 YAML Documents sent to Cohere:`);
        documents.slice(0, 5).forEach((doc, idx) => {
          console.log(`\n--- Document ${idx + 1} ---`);
          console.log(doc);
          console.log('Fields populated:', {
            name: doc.includes('name:') && !doc.includes('name: ""'),
            jobTitle: doc.includes('job_title:') && !doc.includes('job_title: ""'),
            company: doc.includes('company:') && !doc.includes('company: ""'),
            notes: doc.includes('notes:') && !doc.includes('notes: ""'),
            tags: doc.includes('tags:') && !doc.includes('tags: []'),
            linkedin: doc.includes('linkedin:') && !doc.includes('linkedin: ""')
          });
        });
        console.log('');
