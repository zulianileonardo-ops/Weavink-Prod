 ...
 * MIGRATION: 2025-12-04 - Switched from Cohere API to self-hosted embed-server
 *
 * Models:
 * - Embedding: intfloat/multilingual-e5-large (1024 dims, self-hosted)
 * - Reranking: BAAI/bge-reranker-base (self-hosted)
 *
 * Performance vs Cohere:
 * - Cost: $0 (vs $0.08/M tokens + $2/1K reranks)
 * - Latency: ~50ms embed, ~150ms rerank (vs ~300ms each)
 * - Rerank Top-1 Accuracy: 94% (vs 100%)
 * - Rerank Recall@3: 56% (vs 100%)
 */
export const SEMANTIC_SEARCH_CONFIG = {
  // Embedding model - SELF-HOSTED E5-large
  EMBEDDING_MODEL: 'intfloat/multilingual-e5-large', // Self-hosted via embed-server
  EMBEDDING_DIMENSION: 1024, // 1024 dimensions
  ...
  CACHE_TTL_MS: 10 * 60 * 1000, // 10 minutes for search results
  STREAMING_CACHE_TTL_MS: 30 * 60 * 1000, // 30 minutes
  EMBEDDING_CACHE_TTL_S: 86400, // 24 hours for embeddings

  // Rerank configuration - SELF-HOSTED BGE (2025-12-04)
  RERANK_MODELS: {
    // CURRENT: Self-hosted BGE reranker
    BGE_BASE: 'BAAI/bge-reranker-base', // Self-hosted, $0, 94% top-1 accuracy

    // LEGACY: Cohere models (kept for reference)
    MULTILINGUAL_V35: 'rerank-v3.5', // $0.002/request (deprecated)
    ENGLISH: 'rerank-english-v3.0', // $0.001/request (deprecated)
    MULTILINGUAL: 'rerank-multilingual-v3.0', // $0.002/request (deprecated)
  },
  RERANK_MODEL: 'BAAI/bge-reranker-base', // Default model
  RERANK_PROVIDER: 'self-hosted', // Was 'cohere'
  DEFAULT_RERANK_TOP_N: 10,
  MAX_RERANK_CANDIDATES: 20, // Limit for latency optimization

  // Quality metrics (from benchmark vs Cohere)
  EXPECTED_RECALL_AT_3: 0.56, // 56% vs Cohere baseline
  EXPECTED_TOP_1_ACCURACY: 0.94, // 94% top-1 match

  // Qdrant vector database configuration (self-hosted)
  QDRANT_CONFIG: {
    distance: 'Cosine',
  },

  // Batch processing configuration
  BATCH_SIZE: 10, // Larger batches OK for self-hosted (no rate limits)
  BATCH_DELAY_MS: 100, // Minimal delay between batches
...
export const CONFIDENCE_THRESHOLDS = {
  ...
  // LOWERED from 0.60 to 0.45 to show "somewhat related" results
  // e.g., "React developers" query matches "Software Engineer" contacts at 40-53%
  VECTOR_MINIMUM: {
    enterprise: 0.45,  // Keep results with 45%+ vector similarity (was 0.60)
    business: 0.45,    // Keep results with 45%+ vector similarity (was 0.60)
    premium: 0.45,     // Keep results with 45%+ vector similarity (was 0.60)
    pro: 0.45,         // Keep results with 45%+ vector similarity (was 0.60)
    base: 0.45         // Keep results with 45%+ vector similarity (was 0.60)
  },
...
export const RELATIONSHIP_DISCOVERY_THRESHOLDS = {
  // Semantic similarity thresholds (Qdrant vector scores)
  semantic: {
 ...
 * @param {number} score - Similarity score from Qdrant (0-1)
